...
const AuthPage = () => {
      ...
      if (result.error) {
        ...
        switch (errorCode) {
          case "auth/operation-not-allowed":
            setError(
              "この認証方法は有効化されていません。管理者にお問い合わせください。"
            );
            break;
          ...
          case "auth/weak-password":
            setError(
              "セキュリティのため、パスワードは6文字以上で設定してください。"
            );
            break;
    ...
    try {
      ...
      if (error) {
        ...
        switch (errorCode) {
          case "auth/operation-not-allowed":
            setError(
              "Google認証が有効化されていません。Firebase Consoleで有効化してください。"
            );
            break;
  ...
  return (
    <AuthContainer>
      <AuthCard>
        <AuthHeader>
          <AuthTitle>WEALTH TRACKER</AuthTitle>
          <AuthSubtitle>
        ...
        <AuthToggle>
          {isSignUp ? (
            <>
              既にアカウントをお持ちですか？{" "}
              <ToggleButton type="button" onClick={() => setIsSignUp(false)}>
                ログインする
              </ToggleButton>
            </>
          ) : (
            <>
              アカウントをお持ちでない方は{" "}
              <ToggleButton type="button" onClick={() => setIsSignUp(true)}>
                アカウント作成
              </ToggleButton>
            </>
          )}
